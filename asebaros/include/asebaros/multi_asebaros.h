...
class AsebaROS : public Aseba::NodesManager, public rclcpp::Node {
  ...
  typedef std::vector<
      ...
      Publishers;
  typedef std::vector<
      std::map<unsigned, std::shared_ptr<rclcpp::Subscription<AsebaEvent>>>>
      Subscribers;
...
protected:
  ...
  pubFor(const Aseba::UserMessage *asebaMessage);
  ...
  std::string namespace_for_node(unsigned id,
