...
// TODO(Jerome) reenable diagnostic updater if available in foxy
...
void AsebaROS::load(Aseba::BytecodeVector &bytecode, unsigned int node_id) {
  RCLCPP_INFO(get_logger(), "Loading script to node with id %d (%lu bytes)",
              node_id, bytecode.end() - bytecode.begin());
  MessageVector messages;
  auto bytes = std::vector<uint16_t>(bytecode.begin(), bytecode.end());

  sendBytecode(messages, node_id, bytes);
  // TODO(Jerome): why are we not locking here? (or it's just me that removed the lock?)
  for (auto it = messages.begin(); it != messages.end(); ++it) {
    hub.sendMessage((*it).get(), false);
  }
  Aseba::Run msg(node_id);
  hub.sendMessage(&msg, false);
  // TODO(Jerome) why only the suscribers? and not also the publishers here?
  create_subscribers(node_id);
  running_nodes.insert(node_id);
  // TODO(Jerome): should not be when we connect?
  publish_nodes();
}
...
void AsebaROS::load_script_to(unsigned int node_id) {
  ...
  RCLCPP_INFO(get_logger(),
              "Will try to load a script to node with name %s and id %d",
              name.c_str(), node_id);
  // TODO(Jerome): move to bytecode[id] or bytecode[name][id] to allow uploading different scripts
  // to different nodes of the same type
...
void AsebaROS::load_script_to(std::vector<uint16_t> node_ids) {
  ...
  for (auto const &pair : bytecode) {
    ...
    RCLCPP_WARN(get_logger(), "Will load compiled script to nodes with name %s",
                name.c_str());
...
void AsebaROS::read_script_header() {
  if (script_path.empty())
    return;
  RCLCPP_INFO(get_logger(), "Will read script headers in %s",
              script_path.c_str());
  xmlDoc *doc = xmlReadFile(script_path.c_str(), NULL, 0);
  if (!doc) {
    RCLCPP_ERROR(get_logger(), "Cannot read XML from file %s",
                 script_path.c_str());
    return;
  ...
  if (!xmlStrEqual(domRoot->name, BAD_CAST("network"))) {
        ...
        if (xmlStrEqual(domNode->name, BAD_CAST("event"))) {
          ...
          if (!name)
            RCLCPP_WARN(get_logger(),
                        "missing \"name\" attribute in \"event\" entry");
          xmlChar *size = xmlGetProp(domNode, BAD_CAST("size"));
          if (!size)
            RCLCPP_WARN(get_logger(),
                        "missing \"size\" attribute in \"event\" entry");
          // add event
          if (name && size) {
            int eventSize(atoi((const char *)size));
            if (eventSize > ASEBA_MAX_EVENT_ARG_SIZE) {
              RCLCPP_ERROR(get_logger(),
                           "Event %s has a length %d larger than maximum %d",
          ...
          if (name && value) {
            ...
            if (get_parameter(param_name, param)) {
              ...
              set_parameter(rclcpp::Parameter(
                  "script.constants." + constant_name, constant_value));
            }
            std::lock_guard<std::mutex> lock(mutex);
          ...
          RCLCPP_WARN(get_logger(), "Unknown XML node seen in .aesl file: %s",
                      domNode->name);
        }
      }
    }
    ...
    for (size_t i = 0; i < commonDefinitions.events.size(); ++i) {
      pubs.push_back(
          std::map<unsigned, std::shared_ptr<rclcpp::Publisher<AsebaEvent>>>());
      subs.push_back(
          std::map<unsigned,
                   std::shared_ptr<rclcpp::Subscription<AsebaEvent>>>());
    }
    mutex.unlock();
  }
}
...
void AsebaROS::compile_script() {
  if (script_path.empty())
    return;
  RCLCPP_INFO(get_logger(), "Will compile script in file %s",
              script_path.c_str());
  xmlDoc *doc = xmlReadFile(script_path.c_str(), NULL, 0);
  if (!doc) {
    RCLCPP_ERROR(get_logger(), "Cannot read XML from file %s",
        ...
        if (xmlStrEqual(domNode->name, BAD_CAST("node"))) {
          ...
          if (!name) {
            ...
            if (!text) {
              ...
              if (!result) {
                RCLCPP_ERROR(get_logger(),
                             "compilation of %s for node %s failed: %s",
              ...
              RCLCPP_INFO(get_logger(),
                          "Compiled script for nodes with name %s to %lu bytes",
...
void AsebaROS::loadScript(
  ...
  for (auto &constant : req->constants) {
    set_parameter(
        rclcpp::Parameter("script.constants." + constant.name, constant.value));
  }
...
void AsebaROS::getNodeList(
  ...
  std::lock_guard<std::mutex> lock(mutex);
  // transform(nodesNames.begin(), nodesNames.end(),
  // back_inserter(res->node_list),
...
void AsebaROS::getVariableList(
  ...
  if (nodeIt != nodesNames.end()) {
    ...
    RCLCPP_ERROR(get_logger(), "node %s does not exists",
                 req->node_name.c_str());
    return;
    // throw std::invalid_argument("Node does not exists");
  }
...
void AsebaROS::getVariable(
  ...
  if (result) {
    ...
    RCLCPP_ERROR(get_logger(),
                 "read of node %s, variable %s did not return a valid answer "
                 "within 100ms",
...
bool AsebaROS::getVariableInfo(const std::string &nodeName,
  ...
  if (pos == unsigned(-1)) {
    ...
    if (!ok) {
      RCLCPP_ERROR(get_logger(), "Variable %s has unknown size in node %s (%d)",
...
void AsebaROS::nodeDescriptionReceived(unsigned nodeId) {
  ...
  RCLCPP_INFO(get_logger(),
              "Received %s description of a node with name %s and id %d",
  ...
  if (!bytecode.count(name))
    ...
    RCLCPP_INFO(get_logger(), "Script already compiled for name %s",
                name.c_str());
...
void AsebaROS::knownEventReceived(const uint16_t id, const uint16_t nodeId,
                                  const AsebaEvent::SharedPtr event) {
  // does not need locking, does not touch object's members
  if (event->source == 0) {
    RCLCPP_DEBUG(get_logger(), "known event %d received from ROS for node %d",
                 id, nodeId);
    // forward only messages with source 0, which means, originating from this
    // computer
...
// TODO(Jerome): remove or use ... now it is used nowhere
...
AsebaROS::AsebaROS(unsigned port, bool forward,
    ...
    : hub(this, port, forward), rclcpp::Node("asebaros", "",
                                             node_options) {
  ...
  if (manage_single_node)
    RCLCPP_INFO(get_logger(),
                "Will manage only the first valid connected node");

  // does not need locking, called by main
  anonPub =
      create_publisher<AsebaAnonymousEvent>("aseba/anonymous_events", 100);
  auto cb = std::bind(&AsebaROS::eventReceived, this, _1);
  anonSub = create_subscription<AsebaAnonymousEvent>("aseba/anonymous_events",
                                                     100, cb);

  nodes_pub = create_publisher<AsebaNodeList>("aseba/nodes",
                                              rclcpp::QoS(1).transient_local());

  // script
  s.push_back(create_service<LoadScripts>(
      "aseba/load_script", std::bind(&AsebaROS::loadScript, this, _1, _2, _3)));
  // nodes
  s.push_back(create_service<GetNodeList>(
      "aseba/get_node_list",
      std::bind(&AsebaROS::getNodeList, this, _1, _2, _3)));
  s.push_back(create_service<GetNodeIds>(
      "aseba/get_node_ids",
      std::bind(&AsebaROS::getNodeIds, this, _1, _2, _3)));
  s.push_back(create_service<GetNodeName>(
      "aseba/get_node_name",
      std::bind(&AsebaROS::getNodeName, this, _1, _2, _3)));
  // variables
  s.push_back(create_service<GetVariableList>(
      "aseba/get_variable_list",
      std::bind(&AsebaROS::getVariableList, this, _1, _2, _3)));
  s.push_back(create_service<SetVariable>(
      "aseba/set_variable",
      std::bind(&AsebaROS::setVariable, this, _1, _2, _3)));
  s.push_back(create_service<GetVariable>(
      "aseba/get_variable",
      std::bind(&AsebaROS::getVariable, this, _1, _2, _3)));
  // events
  s.push_back(create_service<GetEventId>(
      "aseba/get_event_id",
      std::bind(&AsebaROS::getEventId, this, _1, _2, _3)));
  s.push_back(create_service<GetEventName>(
      "aseba/get_event_name",
  ...
  // TODO(Jerome): How to declare params?
  ...
  if (get_parameter("script.path", file_path_param)) {
    ...
    RCLCPP_INFO(get_logger(), "Initializing with script %s",
                script_path.c_str());
    read_script_header();
  } else {
...
void AsebaROS::processAsebaMessage(Aseba::Message *message) {
  ...
  if (variables) {
    ...
    if (queryIt != getVariableQueries.end()) {
      ...
      RCLCPP_WARN(get_logger(),
                  "received Variables from node %d, pos %d, but no "
...
std::string AsebaROS::namespace_for_node(unsigned id, std::string name) {
  if (namespaces.count(id) == 0) {
    ...
    if (get_parameter("nodes." + name + "." + std::to_string(id), param)) {
      ns = param.as_string();
    } else if (get_parameter("nodes." + name + ".accept_all", accept) &&
               !accept.as_bool()) {
      ns = IGNORE;
    ...
    if (manage_single_node && ns != "-")
      ns = "";
    namespaces[id] = ns;
    RCLCPP_INFO(get_logger(), "Namespace for node with name %s and id %d is %s",
                name.data(), id, ns.data());
  }
...
std::shared_ptr<rclcpp::Publisher<AsebaEvent>>
AsebaROS::pubFor(const Aseba::UserMessage *asebaMessage) {
  ...
  if (pubs[type].count(source) == 0) {
    ...
    pubs[type][source] =
        create_publisher<AsebaEvent>(topic(source, narrow(name)), 100);
  }
...
void AsebaROS::create_subscribers(unsigned node_id) {
  for (size_t i = 0; i < commonDefinitions.events.size(); ++i) {
    ...
    subs[i][node_id] = create_subscription<AsebaEvent>(
        topic(node_id, narrow(name)), 100,
...
void dumpHelp(std::ostream &stream, std::string programName) {
  ...
  stream << "Options:\n";
  stream << "-l, --loop      : ";
  stream << "makes the switch transmit messages back to the send, not only "
            "forward them.\n";
  ...
  while (rclcpp::ok() && !connected) {
    for (auto &target : additionalTargets) {
      ...
      try {
        ...
        RCLCPP_ERROR(asebaROS->get_logger(),
                     "While connecting to target %s: %s", target.c_str(), e.what());
      }
    }
    if (!connected) {
      RCLCPP_WARN(
